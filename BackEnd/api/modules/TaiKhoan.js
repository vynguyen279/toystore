...
class TaiKhoan {
    ...
    static selectAll() {
      ...
      static insert(taiKhoan) {
        return DB.query(`insert into DSTAIKHOAN(TAIKHOAN,MATKHAU,CHUCVU) values ('${taiKhoan.TAIKHOAN}','${taiKhoan.MATKHAU}', '${taiKhoan.CHUCVU}')`)
      }
      static update(TAIKHOAN, MATKHAU, CHUCVU) {
        console.log('update');
        if (MATKHAU.length > 0) {
          return DB.query(`UPDATE DSTAIKHOAN SET MATKHAU = '${MATKHAU}' WHERE TAIKHOAN ='${TAIKHOAN}'`)
        }
        // else {
